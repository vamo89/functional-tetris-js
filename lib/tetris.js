...
function tick (state, time) {
  ...
  state.timeControl.lastDrop = time
  if (collision(state.player, 'D')) {
    if (checkLineComplete(state.player.matrix)) {
      let roundPoints = calculatePointsForLineCompletion(state.player.matrix)
      if (roundPoints === undefined) {
        // Cheating Detected
        console.log('roundPoints undefined')
        return false
      }
      state.player.points += roundPoints
      state.player.matrix = removeCompleteLines(state.player.matrix)
    }
    state.player.piecePos = {x: 0, y: 5}
    state.player.piece = randomPiece()
    state.player.pieceRotation = 0
    if (collision(state.player, '', false)) {
      // End Game
      console.log('END GAME')
      state.continue = false
      return state
    }
    state.player.matrix = createNewPiece(state.player.matrix, state.player.piece)
  } else {
    state.player = move(state.player, 'D')
  }
  return state
}
...
function move (originalPlayer, direction) {
  return innerMove(originalPlayer, newPos(originalPlayer.piecePos, direction))

  function innerMove (originalPlayer, newPos) {
    let player = copy(originalPlayer)
