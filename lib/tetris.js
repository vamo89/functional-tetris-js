module.exports = {
  ...
  gravity: function (originalPlayer) {
    return this.move(originalPlayer, {x: originalPlayer.piecePos.x + 1, y: originalPlayer.piecePos.y})
  },

  move: function (originalPlayer, newPos) {
  ...
  collision: function (player, newPos, remove = true) {
    ...
    let collide = false
    runFnAtPiece(matrix, player.piece, newPos, (matrix, x, y) => { collide = collide || matrix[x] === undefined || matrix[x][y] === undefined || matrix[x][y] })
    return !!collide
  }
